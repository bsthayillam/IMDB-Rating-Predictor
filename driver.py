...
class TestNeuralNet(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.nptrainInp = np.zeros(len(self.train))
    self.nptrainOut = np.zeros(len(self.train))
    self.npcrossInp = np.zeros(len(self.crossVal))
    self.npcrossOut = np.zeros(len(self.crossVal))
    self.nptestInp = np.zeros(len(self.test))
    self.nptestOut = np.zeros(len(self.test))
  #   random.seed(0)
  #   np.random.seed(0)
